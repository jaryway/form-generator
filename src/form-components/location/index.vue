  ...
  <div>
    ...
    <el-dialog :visible.sync="box" append-to-body title="选择位置" width="80%" @close="handleClose">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button v-if="!disabled" icon="el-icon-check" type="primary" @click="handleSubmit">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  name: 'FgLocation',
  props: ['value', 'config', '__slot__', 'disabled', 'placeholder', 'mapCenter'],
  data() {
    const value = this.value || []
    return {
      formattedAddress: '',
      address: value.length === 3 ? value[2] : '',
      poi: {},
      text: '',
      marker: null,
      map: null,
        ...
        if (this.box) {
          this.$nextTick(() =>
            this.init(() => {
              ...
              if (this.mapCenter && this.mapCenter.length > 0) {
