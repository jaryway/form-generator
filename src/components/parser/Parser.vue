...
import { calcFieldDisplayRules, getFieldDisplayRulesMap, getVisibilityMap } from './utils/field-display-rule-helper'
...
function doLinkQuery(scheme, formData, index) {
  ...
  const conditionsCachedKey = [scheme.vModel, index || 0, 'conditions'].join('.')
  const responseCachedKey = [scheme.vModel, index || 0, 'response'].join('.')
  const prevConditions = this.linkQueryMap[conditionsCachedKey] || []
  ...
  // 条件发生了变化，此时才发起请求
  this.$set(this.linkQueryMap, conditionsCachedKey, conditions)
  const onSuccess = (resp) => {
    // 如果查询成功，将结果缓存一下
    this.$set(this.linkQueryMap, responseCachedKey, resp)
    // 如果是关联数据，则需要填充表单
    // 根据填充规则，填充表单
    ...
    return {
      default: (scope) => {
        ...
        if (this.isEdit && editable !== false) {
          ...
          const listeners = buildListeners.call(this, fieldCopy, scheme, index)
          console.log('scopedSlots', field.typeId)
          return (
            <el-form-item label-width='0'>
              <render conf={fieldCopy} on={listeners} key={index} use-value={true} value={scope.row[field.vModel]} />
            </el-form-item>
          )
        }
        ...
        if (parent && parent.typeId === 'QUERY_CHECK') {
          const responseCachedKey = [parent.vModel, index, 'response'].join('.')
          const resp = this.linkQueryMap[responseCachedKey]
          console.log('renderChildForm.scope', field, this.linkQueryMap)
  ...
  const buildColumns = (children) => {
    return (children || []).map((item, idx) => {
      ...
      if (item.typeId === 'QUERY_CHECK') {
        return (
          <el-table-column props={{ label }} key={idx}>
            {(item.linkList || []).map((child, index) => {
              return <el-table-column key={index} props={{ label: child.label, prop: child.vModel }} scopedSlots={scopedSlots(child, false, item)} />
            })}
          </el-table-column>
        )
      }
      ...
      return (
        <el-table-column props={{ label, prop: hasChild ? undefined : item.vModel }} scopedSlots={scopedSlots(item)}>
